...
export function generateStaticParams() {
  return routing.locales.flatMap((locale) =>
    cityPages.map((item) => ({ locale, slug: item.slug }))
  );
}
...
export async function generateMetadata({
  ...
  const path = `/ciudades/${slug}`;
  const canonical = localizedUrl(locale, path);

  return {
    title: `${data.keyword} | ${getBofuCopy(locale).cityHeading}`,
...
export default async function CityBofuPage({
  params,
}: {
  params: Promise<{ locale: string; slug: string }>;
}) {
  ...
  const schema = {
    ...
    areaServed: data.keyword,
    serviceType: "Web dental, SEO local y GEO",
    url: localizedUrl(locale, `/ciudades/${slug}`),
  };
    ...
    <main className="min-h-screen bg-white px-5 py-12 sm:px-8">
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <h1 className="mt-3 text-3xl font-heading font-bold text-foreground sm:text-4xl">
          {copy.cityHeading}: {data.keyword}
        </h1>
        <p className="mt-5 text-base text-muted">
          {copy.bulletSeo} {copy.bulletGeo}
        </p>
